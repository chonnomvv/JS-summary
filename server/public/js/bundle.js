...
var $completeAlls = document.querySelector('#ck-complete-all');
...
function renderFooter() {
  document.querySelector('.completed-todos').innerHTML = todos.filter(function (todo) {
...
function render(res) {
  ...
  todos.forEach(function (todo) {
    html += "<li id = \"".concat(todo.id, "\" class=\"todo-item\">\n     <input class= \"custom-checkbox\" type = \"checkbox\" id = \"ck-").concat(todo.id, "\" ").concat(todo.completed ? 'checked' : '', ">\n     <label for=\"ck-").concat(todo.id, "\">").concat(todo.content, "</label><i class=\"remove-todo far fa-times-circle\"></i>\n     </li>");
  });

  if (menu === 1) {
    html = '';
    menu1Todos = todos.filter(function (_ref) {
      var completed = _ref.completed;
      return !completed;
    });
    menu1Todos.forEach(function (todo) {
      html += "<li id = \"".concat(todo.id, "\" class=\"todo-item\">\n       <input class= \"custom-checkbox\" type = \"checkbox\" id = \"ck-").concat(todo.id, "\" ").concat(todo.completed ? 'checked' : '', ">\n       <label for=\"ck-").concat(todo.id, "\">").concat(todo.content, "</label><i class=\"remove-todo far fa-times-circle\"></i>\n       </li>");
    });
  } else if (menu === 2) {
    html = '';
    menu2Todos = todos.filter(function (_ref2) {
      var completed = _ref2.completed;
      return completed;
    });
    menu2Todos.forEach(function (todo) {
      html += "<li id = \"".concat(todo.id, "\" class=\"todo-item\">\n       <input class= \"custom-checkbox\" type = \"checkbox\" id = \"ck-").concat(todo.id, "\" ").concat(todo.completed ? 'checked' : '', ">\n       <label for=\"ck-").concat(todo.id, "\">").concat(todo.content, "</label><i class=\"remove-todo far fa-times-circle\"></i>\n       </li>");
    });
  }
...
function getTodos() {
  ...
  /* json 타입의 데이터만 뽑아온다(원하는 todos만 뽑아내는 것) */
...
function renderCopy() {
  var html = '';
  todosCopys.forEach(function (todo) {
    html += "<li id = \"".concat(todo.id, "\" class=\"todo-item\">\n    <input class= \"custom-checkbox\" type = \"checkbox\" id = \"ck-").concat(todo.id, "\" ").concat(todo.completed ? 'checked' : '', ">\n    <label for=\"ck-").concat(todo.id, "\">").concat(todo.content, "</label><i class=\"remove-todo far fa-times-circle\"></i>\n    </li>");
  });
  $todos.innerHTML = html;
  document.querySelector('.active-todos').innerHTML = $todos.childElementCount;
...
$todos.addEventListener('click', function (e) {
  if (!e.target.classList.contains('custom-checkbox') && !e.target.classList.contains('remove-todo')) ;else if (e.target.classList.contains('custom-checkbox')) {
    toggleCompleted(e.target.parentNode.id);
    renderFooter();

    if ($nav.children[1].className === 'active') {
      todosCopys = todos.filter(function (todo) {
        return !todo.completed;
      });
      renderCopy();
      renderFooter();
...
$nav.addEventListener('click', function (e) {
  var navChildren = _toConsumableArray($nav.children);

  navChildren.map(function (nav) {
